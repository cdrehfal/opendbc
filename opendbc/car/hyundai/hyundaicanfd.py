...
def create_buttons(packer, CP, CAN, cnt, btn, lda=False):
  values = {
    "COUNTER": cnt,
    "SET_ME_1": 1,
    "CRUISE_BUTTONS": btn,
    "LDA_BTN": 1 if lda else 0,
  }
...
def create_ccnc(packer, CAN, openpilotLongitudinalControl, enabled, hud, leftBlinker, rightBlinker, msg_161, msg_162, msg_1b5,
  ...
  # handing back and 1 when paused; lighting it for all three leaves the cluster claiming openpilot
  # has the wheel for about a second after control has already returned to the driver.
  steering = lfa_icon == 2
  msg_161.update({
    "DAW_ICON": 0,
    "LKA_ICON": 0,
    "LFA_ICON": 2 if steering else 0,
